...
`python scripts/mpa.py --help` 查看命令。`doctor` 检查环境；`init PROJECT` 创建访谈记录；`validate PROJECT` 校验 JSON 与关系；`build PROJECT` 生成草稿；`render PROJECT` 回渲染；`review-template PROJECT` 创建待人工填写审核；`qa PROJECT` 运行门禁；`prepare-delivery-notice PROJECT` 生成待展示的交付通知；通知确实显示后用 `notice-sent --kind delivery --channel ...` 记账；`export PROJECT` 仅在全部通过时输出最终包。
