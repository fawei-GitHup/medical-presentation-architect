...
既有 PPTX 的媒体先做低上下文盘点；索引不会把原始大图写入 JSON 或日志。review 文件可逐批补全，只有明确选中的媒体才导出原图：

```sh
python scripts/mpa.py media-index projects/my-talk source.pptx --batch-size 4
python scripts/mpa.py review-media projects/my-talk --batch-size 4
...
python scripts/mpa.py resume projects/my-talk
python scripts/capture_run.py --output run/build.jsonl --scrub-images --heartbeat 60 --phase build -- python scripts/mpa.py build projects/my-talk
```
...
python scripts/mpa.py source-map-check projects/my-talk
python scripts/mpa.py perceptual-preflight projects/my-talk
python scripts/mpa.py doctor
...
定点修订使用结构化 findings（每项含 `slide_id`、`issue`、`evidence`、`requested_change`、`severity`）：

```sh
python scripts/mpa.py revise projects/my-talk --findings findings.json
```

命令保存目标页 before 图并只使目标页审核及 build/render/相关机器检查失效；按 finding 修改后必须 rebuild、rerender，并复查目标页和整套 contact sheet。脚本不会假装已经完成语义修改或人工复核。
